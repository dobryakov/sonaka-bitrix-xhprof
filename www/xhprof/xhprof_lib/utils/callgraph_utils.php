...
function xhprof_generate_image_by_dot($dot_script, $type) {
  ...
  // Fontconfig (Graphviz) needs writable cache; upstream only passed PATH, which breaks dot under www-data.
  $dotEnv = array(
    'PATH' => getenv('PATH') ?: '/usr/local/bin:/usr/bin:/bin',
    'XDG_CACHE_HOME' => getenv('XDG_CACHE_HOME') ?: '/var/www/.cache',
    'HOME' => getenv('HOME') ?: '/var/www',
  );
  $process = proc_open($cmd, $descriptorspec, $pipes, sys_get_temp_dir(), $dotEnv);
